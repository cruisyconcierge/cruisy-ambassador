...
// --- Configuration ---
// In a real build, use the imported functions from ./lib/api.js
// For this generation, we are keeping the logic self-contained but prepared for API switch.
...
export default function App() {
  ...
  useEffect(() => {
    const savedUser = localStorage.getItem('cruisy_ambassador_session');
    if (savedUser) {
  ...
  const handleLogin = async (userData) => {
    setLoading(true);
    // TODO: Replace with: const data = await loginUser(email, password);
    ...
    setTimeout(() => {
        ...
        setUser(defaultData);
        localStorage.setItem('cruisy_ambassador_session', JSON.stringify(defaultData));
        setLoading(false);
    }, 1000);
  };
  ...
  const handleLogout = () => {
    setUser(null);
    localStorage.removeItem('cruisy_ambassador_session');
  ...
  const updateUser = (updates) => {
    ...
    setUser(updated);
    localStorage.setItem('cruisy_ambassador_session', JSON.stringify(updated));
  };
...
// ... (Academy, Membership, Earnings components remain the same as previous step, just ensure they are included in the file)
...
function ProfileEditor({ user, updateUser }) {
  ...
  // MOCK DATA for now - in production, this comes from src/lib/api.js getItineraries()
  ...
  const filteredActivities = MOCK_ITINERARIES.filter(act => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in zoom-in duration-300">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div><h2 className="text-2xl md:text-3xl font-russo text-slate-800">Edit Page</h2><p className="text-slate-500">Manage your bio and featured itineraries.</p></div>
...
// ... Include Academy, Membership, Earnings, AuthPage, DesktopNavItem from previous responses
function Academy({ user, setActiveTab }) {
  // ... (Same content as previous step, ensuring training modules are displayed)
  return (
    <div className="space-y-6 animate-in fade-in zoom-in duration-300">
...
function Membership({ user, updateUser }) {
  ...
  const plans = [
    {
      ...
      features: ['10% Commission on Activities', '12% on Fury Water Adventures', 'Up to 5% on Stays & Cruises', 'Basic Dashboard Analytics', 'Link Generator Tool', 'Monthly Payouts'],
    ...
    },
    {
      ...
      features: ['Boosted Activity Rates (Up to 15%)', 'Priority on Stays/Cruises', 'Cruisy Academy Access', 'Priority Payouts (Weekly)', 'Verified Profile Badge', 'Advanced Analytics'],
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-12">
       <div className="text-center max-w-2xl mx-auto"><h2 className="text-3xl font-russo text-slate-800 mb-4">Supercharge Your Earnings 🚀</h2><p className="text-slate-500 text-lg">Join the Elite tier to unlock higher commissions, advanced training, and faster payouts.</p></div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto items-start">
         {plans.map((plan) => (
           <div key={plan.id} className={`bg-white rounded-3xl p-8 border transition-all relative ${plan.recommended ? 'border-amber-400 shadow-xl shadow-amber-500/10 scale-100 md:scale-105 z-10' : 'border-slate-200 shadow-sm'} ${plan.disabled ? 'opacity-70 grayscale' : ''}`}>
             ...
             <button onClick={() => updateUser({ plan: plan.id })} disabled={plan.active || plan.disabled} className={`w-full py-4 rounded-xl font-russo text-lg transition-all shadow-lg active:scale-95 ${(plan.active || plan.disabled) ? 'bg-slate-100 text-slate-400 cursor-default shadow-none' : plan.recommended ? 'gold-gradient text-white hover:shadow-amber-500/25' : 'bg-slate-800 text-white hover:bg-slate-900'}`}>{plan.buttonText}</button>
           </div>
         ))}
       </div>
    </div>
...
function AuthPage({ onLogin }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    onLogin({ name: formData.name || 'Alex Smith', email: formData.email, slug: formData.slug || 'alex-travels', bio: 'Just a girl exploring the world, one cruise at a time.', joinedDate: 'Oct 2023' });
  };
      ...
      <div className="md:w-1/2 relative flex flex-col justify-center p-8 md:p-12 lg:p-16 text-white overflow-hidden bg-gray-900">
         ...
         <div className="relative z-10 max-w-lg mx-auto md:mx-0">
           ...
           <div className="space-y-6">
             ...
             <div className="flex items-start gap-4"><div className="bg-white/20 p-3 rounded-xl backdrop-blur-md border border-white/10"><Zap size={24} className="text-white" /></div><div className="w-full"><h3 className="font-russo text-xl text-white">Your Own Booking Page</h3><p className="text-blue-50 text-sm mb-2">Get a custom <strong>cruisytravel.com/you</strong> page to share with followers. We track every booking automatically.</p><button onClick={() => setIsLearnMoreOpen(true)} className="w-full mt-8 py-3 bg-white/20 hover:bg-white/30 text-white border border-white/40 rounded-xl transition-colors flex items-center justify-center gap-2 backdrop-blur-sm text-sm font-bold shadow-lg"><Info size={16} /> Program Details</button></div></div>
           </div>
         </div>
      </div>
